...
class DQN:
    ...
    def _calculate_loss(self, transitions, gamma=0, use_target=False):
        ...
        network_prediction = self.q_network.forward(input_tensor).gather(dim=1, index=actions.unsqueeze(-1)).squeeze(-1)

        if use_target == True:
            future_q =  self.target_network.forward(next_tensor)
            # print(future_q.grad_fn)
            # print(future_q)
            ...
            future_returns = torch.max(future_q, 1).values
...
def q_visual(environment, dqn):
    ...
    path = os.getcwd()
    cv2.imwrite(os.path.join(path, '..\\Figures\\__QValuesImage.png'), q_values_image)
    cv2.waitKey(0)

def greedy_policy_visual(environment, agent):
    agent.reset()
    ...
    path = os.getcwd()
    cv2.imwrite(os.path.join(path, '..\\Figures\\__GreedyPolicyImage.png'), policy_image)
    cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ax.set(xlabel='Episode', ylabel='Loss', title='Loss (MSE wrt reward), against episode number')
    episode_counter = 0
    episodes = []
    losses = []
    ...
    n_steps = 75
    n_episodes = 300
    N = 5 #Number of steps after which target gets updated
    ...
    epsilon_min = 0.1
    epsilon_decay = True
    # epsilon_step = (epsilon - epsilon_min) / n_episodes
    epsilon_step = (epsilon - epsilon_min) / (n_episodes*n_steps)
    do_target = True
    ...
    while episode_counter < n_episodes:
        ...
        for step_num in range(n_steps):
            ...
            if replay_buffer.get_len() >= batch_size:
                ...
                if epsilon_decay and epsilon > epsilon_min and (episode_counter - additional_episodes) >= 0:
        ...
        if loss_avg != 0:
            losses.append(loss_avg/n_steps)
    ...
    if show_q_values: q_visual(environment, dqn)
    if show_best_policy: greedy_policy_visual(environment, agent)
